...
const Quiz = require('../models/Quiz'); // Quiz modelini import et
// const authMiddleware = require('../middleware/auth'); // Sonraki adımda eklenecek

const router = express.Router();

// --- Yeni Quiz Oluşturma ---
// POST /api/quizzes
// !! Bu rotayı korumak için authMiddleware eklenecek !!
router.post('/', /* authMiddleware, */ async (req, res) => {
    const { title, description, questions } = req.body;
    // const userId = req.user.id; // Bu bilgi authMiddleware'den gelecek

    // Geçici olarak userId'yi manuel alalım (test için) - GERÇEK UYGULAMADA KALDIRILACAK!
    const { userId } = req.body;
    if (!userId) {
         return res.status(400).json({ message: 'Test için userId gereklidir (normalde token\'dan alınır)' });
    }
    //------------------------------------------------------------------------------------
